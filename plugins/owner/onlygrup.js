...
export default {
    name: 'onlygrup',
    aliases: ['onlygc', 'onlygroups'],
    description: 'Atur mode bot khusus grup',
    ownerOnly: true,
    execute: async ({ sock, msg, args, config, prefix, command, useLimit }) => {
        ...
        if (input === 'on') {
            if (config.onlyGroup) {
                useLimit()
                await sock.sendMessage(jid, {
                    text: '⚠️ Onlygrup sudah aktif.'
                }, { quoted: msg })
                return
            }

            await settingsDb.setRestrictions({ onlyGroup: true, onlyPrivate: false, onlyOwner: false, onlyPremium: false })
            useLimit()
            await sock.sendMessage(jid, {
                text: '✅ Onlygrup berhasil diaktifkan.'
            }, { quoted: msg })
            return
        }
        ...
        useLimit()
        await sock.sendMessage(jid, {
            text: '✅ Onlygrup berhasil dimatikan.'
        }, { quoted: msg })
    }
}
