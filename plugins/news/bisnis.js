...
const SITEMAP_URL = 'https://www.bisnis.com/sitemap-news.xml'
...
const formatItem = (item, index) => (
    ...
    `× Link: ${item.link}\n` +
    `× Deskripsi: ${truncate(item.description || item.keywords || '-')}`
)

export default {
    name: 'bisnis',
    aliases: [],
    description: 'Bisnis.com',
    execute: async ({ sock, msg, react, useLimit }) => {
        ...
        try {
            ...
            if (!items.length) {
                await react('❌')
                return sock.sendMessage(jid, { text: '⚠️ Tidak ada berita ditemukan dari Bisnis.com.' }, { quoted: msg })
            }
            ...
            if (!firstImage) {
                await react('❌')
                return sock.sendMessage(jid, { text: '❌ Bisnis.com: Tidak ada gambar valid yang bisa diambil untuk ditampilkan.' }, { quoted: msg })
            }
            ...
            await sock.sendMessage(jid, {
                text: `❌ Gagal ambil berita Bisnis.com: ${err.message}`
            }, { quoted: msg })
        }
    }
}
