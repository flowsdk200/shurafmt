...
export default {
    ...
    execute: async ({ sock, msg }) => {
        ...
        text += `• OS: ${os.version() || 'Not available'}\n`
        ...
        text += `• RAM Used: ${formatSize(usedMem)} (${memPercent.toFixed(1)}%)\n`
        text += `• RAM Available: ${formatSize(freeMem)} (${freePercent.toFixed(1)}%)\n`
        text += `• Disk Total: ${formatSize(disk.total)}\n`
        text += `• Disk Used: ${formatSize(disk.used)} (${disk.percent.toFixed(1)}%)\n`
        text += `• Disk Free: ${formatSize(Math.max(0, Number(disk.total || 0) - Number(disk.used || 0)))}\n`
